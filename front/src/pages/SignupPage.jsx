...
const handleNicknameChange = (v) => {
  ...
  if (!validateNick(v)) {
    ...
    setNickWarn("");
    setValid((vd) => ({ ...vd, nick: false })); // 형식 OK, 중복은 미확인 상태
    setMsg((m) => ({ ...m, nick: "" }));
  }
};
...
const handleCheckNicknameDuplicate = async () => {
  ...
  if (!validateNick(v)) {
    alert("닉네임은 3~8자 (한글/영문/숫자)만 가능합니다.");
    return;
  }
  try {
    const { isDuplicate } = await checkDuplicateNickname(v);
    if (isDuplicate) {
      setValid((vd) => ({ ...vd, nick: false }));
      setMsg((m) => ({ ...m, nick: "이미 사용 중인 닉네임입니다." }));
      alert("이미 사용 중인 닉네임입니다.");
    } else {
      setValid((vd) => ({ ...vd, nick: true }));
      setMsg((m) => ({ ...m, nick: "사용 가능한 닉네임입니다." }));
      alert("사용 가능한 닉네임입니다!");
    }
  } catch (err) {
    const em = err?.message || (typeof err === "string" ? err : "") || "닉네임 확인 중 오류가 발생했습니다.";
    alert(em);
  }
};
  ...
  const handleCheckAuthCode = async () => {
    ...
    try {
      ...
      console.error(err);
      setValid((vd) => ({ ...vd, emailAuth: false }));
      const em = err?.message || (typeof err === "string" ? err : "") || "인증 확인 중 오류가 발생했습니다.";
      setMsg((m) => ({ ...m, auth: em }));
    }
  };
  ...
  const handleCheckIdDuplicate = async () => {
    const v = form.loginId.trim();

    if (!validateId(v)) {
      alert("아이디는 영문 대/소문자/숫자 6~16자로 입력해주세요.");
      return;
    }

    try {
      const { isDuplicate } = await checkDuplicateId(v);
      if (isDuplicate) {
      ...
      const em = err?.message || (typeof err === "string" ? err : "") || "아이디 확인 중 오류가 발생했습니다.";
      alert(em);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const em = err?.message || (typeof err === "string" ? err : "") || "회원가입 중 오류가 발생했습니다.";
      alert(em);
      fetchCaptcha();
    }
  };
        ...
        <div>
          <label className="text-sm font-medium">닉네임 (선택)</label>
           <div className="mt-1 flex gap-2">
            ...
            {nickWarn && <p className="text-xs mt-1 text-red-500">{nickWarn}</p>}
          {msg.nick && (
            <p className={`text-xs mt-1 ${valid.nick ? "text-green-600" : "text-red-500"}`}>
              {msg.nick}
            </p>
          )}
        </div>
