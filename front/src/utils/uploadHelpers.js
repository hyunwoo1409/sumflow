...
export async function downloadAllResultsAsZip(doneItems) {
  ...
  const safePart = (s = "", fallback = "Uncategorized") => {
    let x = String(s).trim();
    if (!x) return fallback;
    x = x.replace(/[\\:*?"<>|]/g, "_").replace(/\s+/g, " ");
    return x.slice(0, 80);
  };

  const extractCatFromSummary = (summary = "") => {
    if (!summary) return null;
    const cleaned = summary.replace(/<\|file_separator\|>/g, "");
    const m = cleaned.match(/카테고리\s*[:：]\s*([^\n\r]+)\s*$/m);
    if (!m) return null;
    let cat = (m[1] || "").replace(/[`"'“”‘’»]+/g, "").trim();
    cat = cat.replace(/\s*\/\s*/g, "_");
    if (!cat.includes("_")) cat = `${cat}_일반`;
    return cat;
  };

  const resolveCategoryFolder = (it) => {
    const raw =
      it?.result?.category ??
      it?.result?.category_name ??
      it?.category ??
      it?.categoryName ??
      extractCatFromSummary(it?.result?.summary ?? it?.summary ?? "") ??
      "Uncategorized_일반"; // 기본값

    const oneLevel = String(raw).replace(/\s*\/\s*/g, "_");
    return safePart(oneLevel, "Uncategorized_일반");
  };
  ...
  for (const it of doneItems) {
    ...
    const safeBase = safePart(baseName, "document");

    const catFolder = resolveCategoryFolder(it);      // 예: "법률_법안"
    const itemFolder = `${catFolder}/${safeBase}`;     // ← 문서별 하위 폴더 유지(충돌 방지)

    // 1) OCR JSON
    if (it.result?.ocr) {
      const ocrJson = JSON.stringify(it.result.ocr, null, 2);
      zip.file(`${itemFolder}/${safeBase}_ocr.json`, ocrJson);
    }

    // 2) 요약 TXT
    const summaryRaw = it.result?.summary || "";
    const summaryClean = summaryRaw.replace(/<\|file_separator\|>/g, "").trim();
    if (summaryClean) {
      zip.file(`${itemFolder}/${safeBase}_summary.txt`, summaryClean);
    }

    // 3) 원본 파일
    if (file) {
      zip.file(`${itemFolder}/${file.name}`, file);
    }
  }
...
export function sanitizeCategory(input, { relPath = "", fallback = "Uncategorized" } = {}) {
  const EXT_RX = /\.(pdf|hwp|hwpx|doc|docx|ppt|pptx|xls|xlsx|txt|zip|jpg|jpeg|png)$/i;
  ...
  if (!input) return topFolderOf(relPath) || fallback;

  let s = String(input).trim();
  s = s.replace(EXT_RX, ""); // 확장자 제거

  if (s.includes("/") || s.includes("\\")) {
    s = s.replaceAll("\\", "/").split("/").filter(Boolean)[0] || s; // 경로의 첫 폴더 추출
  }

  if (!s || s.length > 80) return topFolderOf(relPath) || fallback; // 비거나 과도하게 긴 값 방지
  return s;
}